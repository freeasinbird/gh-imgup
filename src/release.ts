 ...
 */
function redactBody(token: string, body: string): string {
 ...
 */
function decodesToToken(value: string, token: string): boolean {
  let current = value;
 ...
 * the literal token, so a tampered field (content_type/state/size) carrying an
 * encoded token would otherwise leak it to stderr/CI logs. If the value holds
 * the token at ANY percent-encoding depth, redact the whole field; otherwise it
 * is safe to echo verbatim for diagnostics.
 */
function redactField(value: unknown, token: string): string {
...
export async function uploadAsset(
  ...
  if (decodesToToken(displayName, token)) {
    throw new Error(
      sanitize(
        token,
